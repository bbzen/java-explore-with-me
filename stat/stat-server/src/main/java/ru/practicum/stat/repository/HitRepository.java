...
public interface HitRepository extends JpaRepository<EndpointHit, Long> {
    ...
    @Query(value = "SELECT new ru.practicum.stat.dto.ViewStatsDto(eh.app, eh.uri, count(eh.uri)) FROM EndpointHit as eh WHERE eh.timestamp >= ?1 AND eh.timestamp <= ?2 GROUP BY eh.app, eh.uri")
    ...
    @Query(value = "SELECT new ru.practicum.stat.dto.ViewStatsDto(eh.app, eh.uri, count(DISTINCT eh.ip)) FROM EndpointHit as eh WHERE eh.uri IN ?1 AND eh.timestamp >= ?2 AND eh.timestamp <= ?3 GROUP BY eh.app, eh.uri")
